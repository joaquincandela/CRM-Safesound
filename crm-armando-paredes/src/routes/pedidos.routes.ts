...
router.post("/", requireAuth, requireRole("ADMIN", "VENTAS"), asyncHandler(controller.crear));
router.get("/", requireAuth, asyncHandler(controller.listar));
router.get("/:id", requireAuth, asyncHandler(controller.obtener));
router.patch("/:id", requireAuth, requireRole("ADMIN", "VENTAS"), asyncHandler(controller.actualizar));
